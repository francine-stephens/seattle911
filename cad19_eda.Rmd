...
2) What is the difference between Onview and Proactive(Officer Initiated)? Is it possible that these two types would be worth combining?
...
#   xlab('Longitude') + 
#   ylab('Latitude') + 
#   ggtitle("911 Events in 2019", subtitle="Seattle, WA")
# 
# ggsave(filename="seattle_points.jpeg", plot=seattle_points, width=6, height=6, units = "in")
...
leaflet() %>%
  ...
  addLegend(
    ...
    pal = prec_pal,
    values = ~total_events,
    title = "2019 Events<br>per Precinct"
  )
...
leaflet() %>%
  ...
  addLegend(
    ...
    pal = zips_pal,
    values = ~events,
    title = "2019 Events<br>per Zipcode"
  )
...
leaflet() %>%
  ...
  addLegend(
    ...
    pal = nhoods_pal,
    values = ~events,
    title = "2019 Events<br>per Neighborhood"
  )
